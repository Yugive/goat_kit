...
var CmdServer = &cobra.Command{
	...
	Long:  "Generate the proto server implementations. Example: goat proto server api/xxx.proto --target-dir=internal/service",
	Run:   run,
}
...
func run(cmd *cobra.Command, args []string) {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Please specify the proto file. Example: goat proto server api/xxx.proto")
		return
	}
	...
	if _, err := os.Stat(targetDir); os.IsNotExist(err) {
		fmt.Printf("Target directory: %s does not exist", targetDir)
		return
	}
	for _, s := range res {
